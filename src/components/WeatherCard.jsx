...
const CityName = styled.h2`
  ...
  font-size:clamp(2rem,8vw,3rem);letter-spacing:0.05em;color:var(--text);line-height:1;word-break:break-word;
...
const SlotWrap  = styled.div`overflow:hidden;display:flex;align-items:center;height:clamp(4.5rem,14vw,8rem);`;
const SlotDigit = styled.span`
  font-family:'Bebas Neue',sans-serif;font-size:clamp(3.5rem,11vw,7rem);line-height:1;
  background:linear-gradient(135deg,#e2e8f0 30%,var(--primary) 100%);
  -webkit-background-clip:text;-webkit-text-fill-color:transparent;display:inline-block;
  animation:${fadeUp} 0.4s var(--ease) both;animation-delay:${p=>p.$i*0.06}s;
`;
...
function Compass({ deg }) {
  ...
  return (
    <CompassWrap>
      <CompassRing $spinning={spinning} viewBox="0 0 54 54" fill="none">
...
function HourlyChart({ forecast, units }) {
  ...
  return (
    <ChartWrap>
      <svg viewBox={`0 0 ${W} ${H+28}`} style={{width:'100%',minWidth:'260px',overflow:'visible'}}>
        <defs>
        ...
        {points.map((p,i)=>(
          <g key={i}>
            ...
            <text x={pad+i*xStep} y={H+18} textAnchor="middle" fill="rgba(148,163,184,0.8)" fontSize="8" fontFamily="DM Mono">{p.label}</text>
          </g>
        ))}
      </svg>
    </ChartWrap>
      ...
      <Card>
        ...
        <InfoGrid>
          {[
            ...
            { i:1, content:<><InfoIcon className="fas fa-droplet"/><InfoLabel>Humidity</InfoLabel><InfoValue>{data.humidity}%</InfoValue></> },
            { i:2, content:<><InfoIcon className="fas fa-gauge"/><InfoLabel>Pressure</InfoLabel><InfoValue>{data.pressure}</InfoValue></> },
            { i:3, content:<><InfoIcon className="fas fa-eye"/><InfoLabel>Visibility</InfoLabel><InfoValue>{data.visibility?`${(data.visibility/1000).toFixed(1)}km`:'N/A'}</InfoValue></> },
          ].map(({i,content})=><InfoItem key={i} $i={i}>{content}</InfoItem>)}
        </InfoGrid>
